...
interface Props {
    ...
    dataToEdit: OfficeProps;
}
export default function EditOffice({ open, setOpen, dataToEdit }: Props) {
    const { data, setData, processing, errors, put, reset } = useForm<OfficeTypes>({
        office_name: dataToEdit.office_name,
        office_code: dataToEdit.office_code,
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        put(route('office.update', dataToEdit.id), {
                ...
                <form className="mt-4 flex flex-col gap-6" onSubmit={submit}>
                    ...
                    <div className="grid gap-4">
                        <div className="grid gap-3">
                            <Label>Name</Label>
                            <Input placeholder="Enter office name" onChange={handleInputChange} value={data.office_name} name="office_name" />
